...
// a conversion factor is not part of lhs
	T_(Trace trc(2,"Bndry(Tok) constructor");)
	Specs& sp = flutspecs();

	T_(trc.dprint("input opt: ",opt);)

	// special case: lhs is a curve id, e.g. vso.1.a and no rhs
	vector<string> toks = string2tok(opt.lhs, '.');
	if (toks.size() > 1 && toks[0] == sp.sid && opt.svec.empty()) {
		curveid = opt.lhs;
		...
		closest = true;
		if (opt.ropt.empty())
			throw runtime_error("incomplete \"closest\" option: "
					"should have the form, e.g. \"sigma=closest{0}\"");
...
// print a summary of a component of a Region (a Bndry) to a string
	ostringstream os;
	double fac{1.0};
...
#ifdef NEVER // no aid
	vector<string> toks = string2tok(cid, '.');
	if (toks.size() < 2)
		throw runtime_error(vastr("invalid curve id: \"",cid,"\""));
	return stoi(toks[1]);
#else // NEVER // no aid
	string::size_type idx = cid.find('.');
	if (idx != string::npos)
		return stoi(cid.substr(0,idx));
	return stoi(cid);
#endif // NEVER // no aid
