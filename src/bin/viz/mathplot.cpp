...
wxBitmap mpLayer::GetColourSquare(int side)
{
    ...
    wxBrush brush(filler, wxBRUSHSTYLE_SOLID);	// EEM
    wxMemoryDC dc;
		...
		wxLogMessage(wxT("mpScaleX::Plot: n: %f -> p = %d"), n, p);
			...
			if ((p >= startPx) && (p <= endPx)) {
				if (m_ticks) { // draw axis ticks
					...
					m_pen.SetStyle(wxPENSTYLE_DOT);	// EEM
					dc.SetPen(m_pen);
					...
					m_pen.SetStyle(wxPENSTYLE_SOLID);	// EEM
					dc.SetPen(m_pen);
				}
	...
	m_desiredYmin 	+= Ay_units;
        ...
        if (event.m_leftDown) {
            if (m_movingInfoLayer == NULL) {
                wxClientDC dc(this);
                wxPen pen(*wxBLACK, 1, wxPENSTYLE_DOT);	// EEM
                dc.SetPen(pen);
...
void mpWindow::Fit(double xMin, double xMax, double yMin, double yMax, wxCoord *printSizeX,wxCoord *printSizeY)
{
	...
	// EEM 23 Nov 2024 Watch out for small Ax or Ay
	...
	m_scaleX = (Ax>tol) ? (m_scrX - m_marginLeft - m_marginRight)/Ax : 1;
	// m_scaleY = (Ay!=0) ? (m_scrY - m_marginTop - m_marginBottom)/Ay : 1; //m_scaleY = (Ay!=0) ? m_scrY/Ay : 1;
	m_scaleY = (Ay>tol) ? (m_scrY - m_marginTop - m_marginBottom)/Ay : 1;
...
#ifdef NEVER // gtk/window.cpp assertion fails
    if (status == false)
    {
        SetScrollbar(wxHORIZONTAL, 0, 0, 0);
        SetScrollbar(wxVERTICAL, 0, 0, 0);
    }
#endif // NEVER // gtk/window.cpp assertion fails
