...
static void psFonts(FILE*), psMarks(FILE*), psEnd(char*);
static void psDot( char *state, int x,int y, int style, int type, int color);
static void psSeg( char *state, int ns, XSegment *seglist,
	int width, int style, int lappr, int color);
static void psText( char *state, int x, int y, char const* text, int just, int style);
...
int width, int height,		/* In microns             */
char *tf,
double ts,
char *af,			/* Title and axis font    */
double as,			/* Title and axis size    */
...
int x, int y,			/* Text position (devs)       */
char const* text,			/* Text itself                */
 ...
 */
{
	...
	if ((style != ui->currentLStyle) || (width != ui->currentWidth)) {
		...
		fprintf (ui->psFile, "%d setlinewidth\n", ui->currentWidth);
	}
	if ((lappr != ui->currentDashStyle) && (style == L_VAR)) {
		if (lappr == 0) {
			fprintf (ui->psFile, "[] 0 setdash\n");
		} else {
			fprintf (ui->psFile, "[%lg] 0 setdash\n",
			    ((double) lappr) * BASE_DASH * VDPI);
		}
		ui->currentDashStyle = lappr;
	}
	fprintf (ui->psFile, "newpath\n");
	fprintf (ui->psFile, "%s setrgbcolor\n", PSColors[color]);
	fprintf (ui->psFile, "  %d %d moveto\n", seglist[0].x1, IY(seglist[0].y1));
	fprintf (ui->psFile, "  %d %d lineto\n", seglist[0].x2, IY(seglist[0].y2));
	for (i = 1;  i < ns;  i++) {
	...
	fprintf (ui->psFile, "stroke\n");

	fprintf (ui->psFile, "grestore\n");
}
 ...
 */
{
	...
	if (ui->currentWidth != PS_ZERO_WBASE * ui->baseWidth) {
		...
		fprintf(ui->psFile, "%d setlinewidth ", ui->currentWidth);
	}

	switch (style) {
	case P_PIXEL:
		fprintf(ui->psFile, "newpath %d %d moveto %d %d %d 0 360 arc fill\n",
	...
	case P_DOT:
		...
		break;
	case P_MARK:
		fprintf(ui->psFile, "%d %d %d mark%d\n",
