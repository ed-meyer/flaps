...
typedef xtb_hret (*Callback)(XEvent*,xtb_data);
...
// for c++11 gray_bits must be unsigned to avoid narrowing: high
// order hex digit must be 7 or less. Then I changed 3rd arg to
// make_stipple to unsigned char*
...
int width, int height)
{
	...
	stip_image = XCreateImage(disp, DefaultVisual(disp, DefaultScreen(disp)),
	    1, XYPixmap, 0, (char*)bits, width, height,
	    bitmap_pad, 0);
 ...
 */
{
	...
	new_info = (struct h_info *) calloc(1,sizeof(struct h_info));
	new_info->func = func;
	new_info->info = info;
 ...
 */
{
	...
	if (!XFindContext(t_disp, win, h_context, (char**)&data)) {
...
char const* text,			/* Text in button    */
...
struct br_info {
	...
	FNPTR( func, xtb_hret, (Window win, int prev, int cur, xtb_data val) );
	xtb_data val;		/* User data          */
	Window *btns;		/* Button windows     */
};

static xtb_hret br_h(
Window win,
int val,
xtb_data info)
/*
 ...
 */
{
	...
	frame->win = XCreateSimpleWindow(t_disp, win, 0, 0, 1, 1,
	    0, back_pix, back_pix);
	info = (struct br_info *) calloc(1,sizeof(struct br_info));
	info->main_win = frame->win;
	info->btns = (Window *) calloc(1,(unsigned) (sizeof(Window) * cnt));
	info->btn_cnt = cnt;
	info->which_one = init;
	info->func = func;
	info->val = val;
	/* the handler is used simply to get information out */
	xtb_register(frame->win, (Callback)0, (xtb_data) info);
	x = BR_XPAD;  
	y = BR_YPAD;
	for (i = 0;  i < cnt;  i++) {
 ...
 */
{
	...
	XSelectInput(t_disp, frame->win, ExposureMask|KeyPressMask|
	    ...
	    ButtonReleaseMask);
	info = (struct ti_info *) calloc(1,sizeof(struct ti_info));
	info->func = func;
	info->val = val;
	info->maxlen = maxchar;
...
char const* text,			/* Replacement text */
xtb_data val)			/* User info        */
/*
...
void xtb_bk_new(
Window win,			/* Parent window */
unsigned width, unsigned height,		/* Size          */
xtb_frame *frame)		/* Returned size */
/*
...
int pad, int inter,			/* Padding and interspace */
unsigned int *rw, unsigned int *rh)			/* Returned size          */
/*
 ...
 */
{
	...
	/* Find parameters */
	for (i = 0;  i < nd;  i++) {
		if ((int)heights[i] > max_height) max_height = heights[i];
		tot_width += widths[i];
	}
 ...
 */
{
	...
	/* Find parameters */
	for (i = 0;  i < nd;  i++) {
		if ((int)widths[i] > max_width) max_width = widths[i];
