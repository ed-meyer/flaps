...
int
run_program (const string& program, const string& options, bool waiton,
		pid_t& childpid, double& cputime, int debug, string const& debugger) {
// Run a program, pipe data to it's stdin, and either wait for it to
// finish or run it in the background.
// Returns:
// - exit status if "waiton" is true; if it is non-zero a text
//   message is available from exitstatus() (text.h)
// - 0 if "waiton" is false and exec is successful, error status if not.
//   The only time childpid will be set to the child process is if waiton=false
	Trace trc(1,"run_program");
	int rval{0};

	fflush(stdout);

	// "program" may contain an aid in the form prog:aid
	// split it off
	string::size_type idx = program.find(':');
	string prog{program};
	string aid;
	if (idx != string::npos) {
		aid = program.substr(idx+1);
		prog = program.substr(0,idx);
	}

	// If DEBUG is set send stderr for the process
	// to it's own file "prog.err" on FTMP
	//!! string debug = getEnv("DEBUG");
	string errfile;
	vector<string> env;
	//!! if (!debug.empty()) 
	// XXX how to get debug into the prog? -d x?
	if (debug > 0) {
		char const* ftmp = getenv("FTMP");
		static int instance = 0;
		instance++;
		ostringstream os;
		if (ftmp != nullptr)
			os << ftmp << string("/") <<  prog << "." << instance << ".err";
		else
			os << prog << "." << instance << ".err";
		errfile = os.str();
		// XXX put in env until we figure out a better way
		putEnv(vastr("DEBUG=",debug));
		//!! env.push_back(vastr("DEBUG=",debug));
	}

	// put prog and aid into arg
	vector<string> arg;
	// debugger is the first arg - it may be multiple tokens, e.g.
	// valgrind --tool=callgrind
	//!! string debugger = getEnv("DEBUGGER");
	if (!debugger.empty()) {
		trc.dprint("got debugger: ",debugger);
		vector<string> toks = string2tok(debugger, " ");
		for (auto& ti : toks)
			arg.push_back(ti);
	}
	...
	// fork/execvp and wait or run in background
	int status;
	string outfile;
	auto start = chrono::steady_clock::now();
	status = exec (arg, env, options, outfile, errfile, waiton);
	auto diff = chrono::steady_clock::now() - start;
	auto sec = chrono::duration_cast<chrono::milliseconds>(diff);
	cputime = sec.count()/1000.0;

	// if exec waited for it to finish it returned the exit status,
	// otherwise it returns the childpid
	if (waiton) {
		childpid = 0;
		rval = status;
	} else
...
exec (const vector<string>& arg, const vector<string>& env,
   ...
   int pfd[2];
	...
	trc.dprint("my pid ",getpid()," args ",arg);
	trc.dprint(env.size()," extra env");
	trc.dprint(", data <",data, "> out file<",outfile,"> errfile<",errfile);
	trc.dprint("> wait? ",wait, " args ",arg);
   ...
   if (!data.empty()) {
		...
		trc.dprint("opened fd ",pfd[0]," and ",pfd[1]," for piping data");
	}
	...
	// fork: returns 0 in child, pid of child in parent, -1 on error
	pid_t childpid = fork();
      ...
      if (!data.empty()) {
			...
			if (pfd[1] != STDIN_FILENO)
				if (close(pfd[1]) == -1)
					throw std::system_error(errno, std::generic_category(),
						vastr("cannot close pipe to ",path));
#ifdef NEVER // leave cin open for interactive
      } else {
			trc.dprint("no data to send: redirect stdin to /dev/null");
			if (freopen("/dev/null", "r", stdin) == 0)
				flaps::warning("cannot redirect cin for ",path,": ",strerror(errno));
#endif // NEVER // leave cin open for interactive
		}
		trc.dprint("calling execvp argv<",arg,">");
   ...
   trc.dprint("Process id for <",path,"> is ",childpid);
   ...
   if (!data.empty()) {
		...
		trc.dprint("writing ",data.size(),"-char data to childs input pipe");
	...
	if (wait) {
		...
		if (rval != -1) {
			trc.dprint(getpid(),": waitpid(",childpid,") exit status 0x",status);
			rval = status;
		}
	} else {
		rval = childpid;
...
addToEnvironment(char* const *envp) {
	...
	for (i=0; envp[i]; i++) {
		ep = strdup(envp[i]);
		trc.dprint("added \"",ep,"\" to environment");
		putenv(ep);
	}
}
...
// Returns the full path of a command, or an empty string if not found
   ...
   if (path == nullptr) {
	...
	string pathstr(path);
	int k{0};
   trc.dprint("searching for ",cmd," on path ",pathstr);
	...
	for (auto& pi : paths) {
		...
		if (access(testpath.c_str(), X_OK) == 0) {
			...
			if (S_ISREG(statbuf.st_mode)) {
				trc.dprint("returning <%s>",testpath);
				if (place != nullptr)
					*place = k;
				return testpath;
			} else {
...
void
write_gdbin(const string& options, const string& debugger) {
	// XXX is this used?
	// write the options to a file $FTMP/gdb.in
	char const* ftmp = getenv("FTMP");
	ostringstream os;
	os << ftmp << "/gdb.in";
	string input(os.str());
	ofstream input_file(input, std::ios::trunc);
	input_file << options;
	// ... and a .gdbint file
	if (debugger == "gdb") {
		ofstream gdbinit(".gdbinit", std::ios::trunc);
		gdbinit << "break main\n";
		// gdbinit << "run " << " <" << input;
	}
}
